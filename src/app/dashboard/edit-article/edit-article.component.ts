...
export class EditArticleComponent implements OnInit {
  ...
  getArticle(key: string): void {
    this.dashboardService.getArticleByKey(key).subscribe((article: Article) => {
      if (article === null) {
  ...
  updateArticle(): void {
    this.saved = false;
    this.dashboardService.updateArticle(this.article).subscribe(res => {
      this.article = res;
      this.saved = true;
    });
  }
  ...
  deleteArticle(): void {
    ...
    const deletionConfirmed = confirm(`Are You sure, You want to delete ${this.article.title} article?`);
